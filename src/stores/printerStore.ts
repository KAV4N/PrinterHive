...
export const usePrinterStore = defineStore('printer', {
  ...
  actions: {
    ...
    simulatePrint(steps: number) {
      if (this.printing) {
        if (this.currentStep >= steps) {
          ...
          this.bedTemperature >= this.bedTarget - 2.5 &&
          this.toolTemperature >= this.toolTarget - 2.5
        ) {
          ...
          this.speeds.z =
            this.currentLayer === Math.floor(this.position.z / this.layerHeight) ? 0 : 5
    ...
    updateTemperatures() {
      ...
      this.bedTemperature = this.calculateNewTemperature(this.bedTemperature, this.bedTarget, 6.75)
    ...
    calculateNewTemperature(current: number, target: number, rate: number): number {
      if (target > current) {
        ...
        return Math.max(Math.max(target, this.roomTemperature), current - coolRate)
      }
